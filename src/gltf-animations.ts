...
// new GLTFLoader().load("models/exo$@walk.glb", (gltf) => {
...
async function loadExo() {
    ...
    const [eve, idle, run, stand] = await Promise.all([
        loader.loadAsync("models/exo$@walk.glb"),
        loader.loadAsync("models/exo@capoera.glb"),
        loader.loadAsync("models/exo@run.glb"),
        loader.loadAsync("models/exo@stands.glb"),
    ]);
    ...
    activeAction = animationActions["stand"];
...
function animate() {
    ...
    if (keyMap["KeyW"]) {
        ...
        //idle
        if (activeAction != animationActions["stand"]) {
            activeAction.fadeOut(0.5);
            animationActions["stand"].reset().fadeIn(0.25).play();
            activeAction = animationActions["stand"];
            toSpeed = 0;
        }
    }
