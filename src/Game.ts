...
export default class Game {
    ...
    async init() {
        ...
        const gui = new GUI();
        gui.add(this.rapierDebugRenderer, "enabled").name(
            "Rapier Degug Renderer"
        );
        ...
        this.pendulums.push(
            new Pendulum(this.scene, this.world, [-5, 8, 2.5], Math.PI / 2)
        );

        this.pendulums.push(
            new Pendulum(this.scene, this.world, [-5, 8, 5], Math.PI / 2)
        );
    ...
    update(delta: number) {
        ...
        this.eventQueue?.drainCollisionEvents((handle1, handle2, started) => {
            if (started) {
                ...
                if (
                    [handle1, handle2].includes(this.finish?.handle as number)
                ) {
